...
public:
	// constructors
	DynArray(void);													// constructs the class by default --------------------------------(1st DAY)
	DynArray(size_t size);												// creates the array with a defined size
	DynArray(size_t size, const int &value);							// creates and initializes the elements of the array with a value -------------------------------(1st DAY)
	...
	// iterator methods
	int* begin(void) const;											// returns an iterator to the first element------------------------------------(1st DAY)
	int* end(void) const;											// returns an iterator to the last element------------------------------------(1st DAY)
	...
	friend bool operator== (const DynArray& lhs, const DynArray& rhs);	// checks if 2 arrays are equal ----------------------------------------(1st DAY)
...
private:
	...
	static void fill(int *first, int *last, int value);		// assigns the given value to the elements in the range [first, last) --------------------------------(1st DAY)
