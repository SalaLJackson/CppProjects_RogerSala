...
public:
	// constructors
	DynArray(void);													// constructs the class by default --------------------------------(1st)
	//DynArray(size_t size);												// creates the array with a defined size
	DynArray(size_t size, const int &value);							// creates and initializes the elements of the array with a value -------------------------------(1st)
	...
	// iterator methods
	int* begin(void) const;											// returns an iterator to the first element------------------------------------(1st)
	int* end(void) const;											// returns an iterator to the last element------------------------------------(1st)
	...
	int& operator[]	(size_t n) const;							// returns a reference to the element at position n in the array ------------------------------(1st)
	...
	void push(const int &val);										// adds a new element at the end of the array, after its current last element---------------------------(2nd)
	...
	friend bool operator== (const DynArray& lhs, const DynArray& rhs);	// checks if 2 arrays are equal ----------------------------------------(1st)
...
private:
	...
	static void fill(int *first, int *last, int value);		// assigns the given value to the elements in the range [first, last) --------------------------------(1st)
